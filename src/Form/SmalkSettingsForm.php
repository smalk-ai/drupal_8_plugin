...
class SmalkSettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['features']['tracking_enabled'] = [
      ...
      '#default_value' => $config->get('tracking_enabled') !== NULL ? $config->get('tracking_enabled') : TRUE,
      '#states' => [
    ...
    $form['features']['ads_enabled'] = [
      ...
      '#default_value' => $config->get('ads_enabled') !== NULL ? $config->get('ads_enabled') : TRUE,
      '#states' => [
        'disabled' => [':input[name="enabled"]' => ['checked' => FALSE]],
      ],
    ];
    ...
    $form['advanced']['exclude_admin_pages'] = [
      ...
      '#default_value' => $config->get('exclude_admin_pages') !== NULL ? $config->get('exclude_admin_pages') : TRUE,
    ];
...
&lt;div smalk-ads id="bottom-ad"&gt;&lt;/div&gt;</pre>
   ...
   */
  public function fixTextFormats(array &$form, FormStateInterface $form_state) {
    $updated_count = $this->updateTextFormats();
    $ckeditor_count = $this->updateCkeditor4Configs();
    ...
    if ($ckeditor_count > 0) {
      $messages[] = $this->t('Updated @count CKEditor configuration(s) to allow smalk-ads in Source Editing.', [
    ...
    if (!empty($messages)) {
      ...
      $this->messenger()->addStatus($this->t('If smalk-ads is still being stripped, make sure "Source" editing is enabled in your CKEditor toolbar.'));
    }
  }

  /**
   * Updates CKEditor 4 editor configurations to allow smalk-ads.
   *
   * @return int
   *   Number of editor configurations updated.
   */
  protected function updateCkeditor4Configs() {
    $updated_count = 0;
    ...
    foreach ($editors as $editor) {
      // Only process CKEditor (4) editors.
      ...
      // Add extraAllowedContent for smalk-ads attribute.
      $extraAllowed = isset($settings['plugins']['stylescombo']['styles']) ? $settings['plugins']['stylescombo']['styles'] : '';

      // We can't easily modify extraAllowedContent through entity API,
      // but the hook_editor_js_settings_alter() in smalk.module handles this.
      // This method is mainly for text format updates.
    }
   ...
   */
  protected function updateTextFormats() {
    $updated_count = 0;
    ...
    foreach ($formats as $format) {
      ...
      $allowed_html = isset($config['filters']['filter_html']['settings']['allowed_html']) ? $config['filters']['filter_html']['settings']['allowed_html'] : '';
      ...
      if ($updated_allowed_html !== $allowed_html) {
        ...
        $existing_settings = isset($config['filters']['filter_html']['settings']) ? $config['filters']['filter_html']['settings'] : [];
        ...
        $format->setFilterConfig('filter_html', [
          'status' => TRUE,
          'weight' => isset($config['filters']['filter_html']['weight']) ? $config['filters']['filter_html']['weight'] : -10,
   ...
   */
  protected function addAttributeToTag($allowed_html, $tag, $attribute) {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($workspaceInfo) {
      $config->set('workspace_key', isset($workspaceInfo['key']) ? $workspaceInfo['key'] : '');
      $config->set('workspace_name', isset($workspaceInfo['name']) ? $workspaceInfo['name'] : '');
      $config->set('publisher_activated', isset($workspaceInfo['publisher_activated']) ? $workspaceInfo['publisher_activated'] : FALSE);
    }
    ...
    $this->messenger()->addStatus($this->t('Smalk settings saved. Workspace: @name', [
      '@name' => isset($workspaceInfo['name']) ? $workspaceInfo['name'] : 'Unknown',
    ]));
  }
   ...
   */
  protected function fetchWorkspaceInfo($apiKey) {
    try {
      ...
      if ($statusCode === 200) {
        ...
        if (is_array($data)) {
          ...
          if (!empty($data) && isset($data[0]) && is_array($data[0]) && isset($data[0]['id'])) {
            $projectData = $data[0];
          }
          // Fallback: Check if it's a single object at root level
          elseif (isset($data['id']) && isset($data['name']) && !isset($data[0])) {
            $projectData = $data;
          }
        }
        ...
        if ($projectData && isset($projectData['id'])) {
          return [
            ...
            'name' => isset($projectData['name']) ? $projectData['name'] : '',
            'publisher_activated' => isset($projectData['publisher_ads_enabled']) ? $projectData['publisher_ads_enabled'] : FALSE,
          ];
        }
